...
warning off
close all
data = ((readtable('.\asthma_disease_data.csv'))); 
features = [data.PollutionExposure data.PollenExposure data.DustExposure data.PetAllergy ];
% columnIndex = 13;
...
output = categorical(data.Diagnosis); %7674x1
%%
...
Y_test = categorical(output(test(cv), :));
...
[testPred,probs] = classify(net,X_test);
accuracy = mean(testPred == Y_test)
confusionchart(testPred,Y_test);
% Calculate RMSE
